...
import { View, Text, StyleSheet, ScrollView, Platform, KeyboardAvoidingView, Alert, ActivityIndicator } from 'react-native';
...
const UpdateExpense = ({ onBack, onCreateExpense, onCancel }) => {
    const route = useRoute();
    const expenseId = route.params.id;
    const [formData, setFormData] = useState({
        asset: route.params.asset.id,
        expenseType: route.params.expenseType,
        date: new Date(route.params.date),
        frequency: route.params.frequency.toString(),
    ...
    const handleCreateExpense = () => {
        if (validateForm()) {
            ...
            updateExpense(expenseId, userId, data)
                .then(data => {
                    if (data.success) {
                        Alert.alert('Update Expense', 'Expense updated successfully.');
                    }
                    else {
                        Alert.alert('Update Expense', 'Some error occurred while updating a expense.');
                    }
                    setLoading(false);
                })
                .catch(error => {
                ...
                <ScrollView contentContainerStyle={{ paddingVertical: 20, paddingBottom: 100 }} style={{ flex: 1, backgroundColor: '#fff' }}>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.sectionContent}>
                            <TextInput
                                label="Date"
                                value={new Date(formData.date).toDateString()}
                                editable={false}
                                mode="outlined"
                                theme={inputTheme}
                                style={styles.input}
