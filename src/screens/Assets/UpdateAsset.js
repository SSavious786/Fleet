...
import { createAsset, getLocations, updateAsset } from '../../api/assets';
import UpdateAssetDetails from '../../../components/assets/UpdateAssetDetails';
import UpdateLifecycleDetails from '../../../components/assets/UpdateLifecycleDetails';
import UpdateFinancialDetails from '../../../components/assets/UpdateFinancialDetails';
import { useRoute } from '@react-navigation/native';

const UpdateAsset = ({ navigation }) => {
    ...
    const submitCompleteAsset = async () => {
        try {
            ...
            const completeAssetData = {
                ...
                "vin": assetData.vin || '',
                "licensePlate": assetData.licensePlate || '',
                "locationId": parseInt(assetData.locationId) || 0,
                "vehicleType": getVehicleTypeEnum(assetData.vehicleType),
                "year": parseInt(assetData.year) || 0,
                "make": assetData.make || '',
                "model": assetData.model || '',
                ...
                "owner": assetData.owner || '',
                "inServiceDate": lifecycleData.inServiceDate || new Date().toISOString(),
                "inServiceOdometer": parseInt(lifecycleData.inServiceOdometer) || 0,
                "estimatedServiceLifeInMonths": parseInt(lifecycleData.estimatedServiceLifeInMonths) || 0,
                "estimatedServiceLifeInMeters": parseInt(lifecycleData.estimatedServiceLifeInMeters) || 0,
                "estimatedResaleValue": parseFloat(lifecycleData.estimatedResaleValue) || 0,
                "companyID": parseInt(companyId),
            ...
            updateAsset(assetId, completeAssetData)
                .then((data) => {
                    console.log(data)
                    if (data.success) {
                        Alert.alert('Update Asset', `Asset updated successfully!`);
                    }
                    else {
                        Alert.alert('Update Asset', `An error occured while updating asset. Please try again.`);
                    }
                    setLoading(false);
                })
                .catch(err => {
    ...
    return (
        ...
        >
            ...
            <View style={{ flex: 1 }}>
                ...
                {activeTab === 1 ? (
                    <UpdateAssetDetails onSubmitSuccess={handleDetailsSubmit} assetDetails={assetDetails} />
                ) : activeTab === 2 ? (
                    <UpdateLifecycleDetails assetData={assetData} onSubmitSuccess={handleLifecycleSubmit} assetDetails={assetDetails} />
                ) : (
                    <UpdateFinancialDetails assetData={assetData} lifecycleData={lifecycleData} onSubmitSuccess={handleFinancialSubmit} assetDetails={assetDetails} />
                )}
            </View>
        </KeyboardAvoidingView>
    );
