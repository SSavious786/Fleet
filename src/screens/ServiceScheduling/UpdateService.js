...
import { createUserService, updateUserService } from '../../api/services';
import { useRoute } from '@react-navigation/native';

function UpdateService({ onSubmitSuccess, onTabChange }) {
    const route = useRoute();
    const serviceId = route.params.id;
    const [formData, setFormData] = useState({
        asset: route.params.assetID,
        serviceTask: route.params.serviceTask,
        serviceType: route.params.status,
        assignedTo: route.params.assignedToUserID,
        timeInterval: route.params.timeInterval.toString(),
        timeDueSoonThreshold: route.params.timeDueSoonThreshold.toString(),
        primaryMeterThreshold: route.params.primaryMeterInterval.toString(),
        primaryMeterDueSoonThreshold: route.params.primaryMeterDueSoonThreshold.toString(),
    });
    ...
    const onSubmit = () => {
        if (validate()) {
            ...
            updateUserService(serviceId ,userId, data)
                .then(res => {
                    if (res.title == 'Duplicate Service Found') {
                        Alert.alert('Update Service', res.description);
                    }
                    else if (res.success) {
                        Alert.alert('Update Service', 'Service updated successfully.');
                    }
                    else {
                        Alert.alert('Update Service', 'Some error occurred while creating a service.');
                    }
                    setLoading(false);
                })
                .catch(error => {
