...
const AddEquipment = ({ onSubmitSuccess }) => {
    ...
    const [warrantyExpiration, setWarrantyExpiration] = useState(new Date());
    ...
    const [estResaleValue, setEstResaleValue] = useState('');
    const [outOfServiceDate, setOutOfServiceDate] = useState(new Date());
    ...
    const onSubmit = () => {
        if (validate()) {
            ...
            createEquipment(userId, data)
                .then(eqp => {
                    if (eqp.title == 'Duplicate Serial Number') {
                        Alert.alert('Create Equipment', eqp.description);
                    }
                    else if (eqp.success) {

                        Alert.alert('Create Equipment', 'Equipment created successfully.');
                    }
                    else {
                        Alert.alert('Create Equipment', 'Some error occurred while creating a equipment.');
                    }
                    setLoading(false);
                })
                .catch(error => {
