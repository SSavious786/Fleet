...
function UpdateAssetDetails({ onSubmitSuccess, assetDetails }) {
    const [assetName, setAssetName] = useState(assetDetails.assetName);
    const [vin, setVin] = useState(assetDetails.vin);
    const [licensePlate, setLicensePlate] = useState(assetDetails.licensePlate);
    const [locations, setLocations] = useState([]);
    // const [location, setLocation] = useState(assetDetails.location != null ? assetDetails.location.id : 0);
    const [location, setLocation] = useState(assetDetails.location == null ? 0 : assetDetails.location.id);
    const [vehicleType, setVehicleType] = useState(assetDetails.vehicleType);
    const [fuelType, setFuelType] = useState(assetDetails.fuelType);
    const [year, setYear] = useState(assetDetails.year.toString());
    const [make, setMake] = useState(assetDetails.make);
    const [model, setModel] = useState(assetDetails.model);
    const [trim, setTrim] = useState(assetDetails.trim);
    const [registrationState, setRegistraionState] = useState(assetDetails.registrationState);
    const [status, setStatus] = useState(assetDetails.status);
    // Assignment section defaults
    const [operator, setOperator] = useState('Active');
    const [owner, setOwner] = useState('Active');
    const [images, setImages] = useState(assetDetails.assetImages);
        ...
        <ScrollView contentContainerStyle={{ paddingVertical: 20, paddingBottom: 100 }} style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <View style={styles.section}>
                ...
                <View style={styles.sectionContent}>
                    ...
                    {Array.isArray(images.$values) && images.$values.length > 0 && (
                        <View style={styles.imagePreviewContainer}>
                            {images.$values.map((img, idx) => {
                                return (
                                    <View key={idx} style={styles.imagePreview}>
                                        <Image source={{ uri: 'https://mytrackio-hcb6gmdtgvhhd4a0.canadacentral-01.azurewebsites.net' + img.imagePath }} style={styles.previewImage} />
                                    </View>
                                )
                            })}
                        </View>
