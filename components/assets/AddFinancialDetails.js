...
function AddFinancialDetails({ onSubmitSuccess, onBack, initialData = {} }) {
    const [purchaseVendor, setPurchaseVendor] = useState('');
    const [purchasePrice, setPurchasePrice] = useState('');
    // Purchase Date state and picker
    const [purchaseDate, setPurchaseDate] = useState(new Date());
    const [showDatePicker, setShowDatePicker] = useState(false);
    const [showFirstPaymentDatePicker, setShowFirstPaymentDatePicker] = useState(false);
    ...
    const [financialType, setFinancialType] = useState('None');
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.sectionContent}>
                            ...
                            <TextInput
                                ...
                                activeOutlineColor={!!errors.End && (touched.End || touched.submit) ? 'red' : '#0acf97'}
                                onBlur={() => setTouched(t => ({ ...t, End: true }))}
                                right={<TextInput.Icon icon="calendar" onPress={() => showLeaseEndDatePicker(true)} forceTextInputFocus={false} />}
                            />
